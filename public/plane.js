class Plane {
  ...
  land() {
    if (this.moving) {
      if (this.waypoint <= 1) {
        ...
        this.vel = p5.Vector.lerp(this.vel, target.sub(this.pos).setMag(this.speed), 0.02)
        this.vel.setMag(this.speed)
        this.height -= this.descendRate
        this.height = constrain(this.height, 0.5, 1.5)
        this.speed -= 0.0003
        this.speed = constrain(this.speed, 0.4, 1.5)

      }

      if (this.pos.dist(this.landingPath[this.waypoint]) < 50) {
        this.waypoint++
  ...
  show() {
    ...
    image(plane, 0, 0, plane.width * this.height, plane.height * this.height)
